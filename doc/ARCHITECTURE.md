...
*   **DirectorSystem**: Logic for Dialogue Graph traversal and state.
*   **ProgressionSystem**: Logic for XP, Leveling, and Evolution.
...
*   **EncounterManager** (*Legacy/Stub*): Generates enemies. Logic largely moved to `EncounterAdapter`.
...
### 7.2. Action Pipeline
*   **Exploration**: Uses `Game_Action` (`src/objects/action.js`) to apply item effects.
*   **Combat**: `BattleSystem` implements its own execution logic (`executeAction`) using plain data objects, bypassing `Game_Action`.
*   **Unified Effects**: Both pipelines delegate specific changes (Damage, Healing) to `EffectSystem.apply()`.

```mermaid
flowchart LR
    Start([Action]) --> Select{Context}
    Select -->|Exploration| GA[Game_Action.apply]
    Select -->|Combat| BS[BattleSystem.executeAction]

    GA --> EffectSys[EffectSystem.apply]
    BS --> EffectSys
...
*   **Encounter Logic**: `EncounterManager` is a legacy stub. Logic is primarily in `EncounterAdapter`.

...
*   **EncounterManager**: Handles enemy generation. Scheduled for eventual migration to `src/engine/systems/`.
...
### 7.2. Action Pipeline
The primary combat loop in `BattleSystem` uses an internal execution pipeline. `Game_Action` is primarily used for UI-initiated actions (items) or in `Scene_Map`.

1.  **Planning**: `BattleSystem.planRound()` generates plain action objects (`{ subject, skillId, target }`).
2.  **Execution**: `BattleSystem.executeAction(state, action)` processes these objects.
3.  **Effect Resolution**: Calls `EffectSystem.apply()` directly.
4.  **Animation**: Returns `Event[]` which `Scene_Battle` iterates to play animations/logs.

```mermaid
flowchart LR
    Start([Turn Start]) --> Select{Action Type}
    Select -->|Attack| Atk[BattleSystem Logic]
    Select -->|Skill| Skill[BattleSystem Logic]
    Select -->|Item| Item[BattleSystem Logic]

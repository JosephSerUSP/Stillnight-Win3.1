...
*   **BattleSystem**: Pure logic for Turn Order and AI decisions.
...
*   **Role**: Replaced by `ExplorationSystem` (Logic) and `ExplorationState` (Data). `ExplorationAdapter` implements the interface consumed by `Scene_Map`.
...
*   **Logic**:
    ...
    2.  `executeAction()`: Performs individual actions.
*   **Flow**:
    *   Pre-round: Command Input.
    *   Loop: The consumer (e.g., `BattleAdapter` or `Scene_Battle`) iterates through the queue generated by `planRound`.
...
3.  **Effect Resolution**: Delegates to `EffectSystem.apply()` which returns a single `Event` object.
...
*   **Logic Separation**: The migration of Battle, Exploration, and Interpreter logic to `src/engine/` is complete. `Game_Map` is deprecated in favor of `ExplorationSystem`. The focus is now on cleaning up remaining coupling in `Game_Battler` and `DungeonGenerator`.

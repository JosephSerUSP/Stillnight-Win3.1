...
*   `ELEMENT_CHANGE`: Overrides the battler's elemental alignment.
*   `eleAdd`: [Planned / Not Implemented] Adds an element to the battler's alignment.
*   `actionMod`: [Planned / Not Implemented] Modifies properties of the battler's actions (e.g., speed boost).

### 3.3. Triggered Traits
Traits can listen for specific triggers (Events) and execute **Effects**:
*   `SYMBIOSIS`: Trigger: "turnStart". Effect: Heals neighbors.
*   `PARASITE`: Trigger: "turnStart". Effect: Drains HP from neighbors.
*   `Mug`: [Planned / Not Implemented] Trigger: "On Damage Dealt". Effect: Gain Gold = Damage.
...
*   `mpd` (MP Drain): [Planned / Not Implemented] Amount of MP drained from Summoner per action.

...
export class MidiParser {
  ...
  parse() {
    ...
    this.readInt32(); this.readInt16(); const nTracks = this.readInt16(); const division = this.readInt16();
    const tracks = []; let tracksFound = 0;
    while (tracksFound < nTracks && this.pos < this.data.length) {
      if (this.pos + 8 > this.data.length) break;
      const chunkType = this.readString(4); const chunkLen = this.readInt32();
      if (chunkType === "MTrk") {
        const startPos = this.pos; const events = []; let ticks = 0; let runningStatus = 0;
        while (this.pos < startPos + chunkLen) {
          const deltaTime = this.readVarInt(); ticks += deltaTime; let eventType = this.data[this.pos]; let event = { ticks };
          if (eventType >= 0x80) { this.pos++; runningStatus = eventType; } else eventType = runningStatus;
          if (eventType === 0xff) { const metaType = this.readInt8(); const len = this.readVarInt(); if (metaType === 0x51) { const microseconds = (this.data[this.pos] << 16) | (this.data[this.pos + 1] << 8) | this.data[this.pos + 2]; event.type = "tempo"; event.microsecondsPerBeat = microseconds; } else if (metaType === 0x2f) event.type = "end"; this.pos += len; }
          else if (eventType === 0xf0 || eventType === 0xf7) { const len = this.readVarInt(); this.pos += len; }
          else { const command = eventType & 0xf0; const channel = eventType & 0x0f; event.channel = channel; if (command === 0x90) { event.note = this.readInt8(); event.velocity = this.readInt8(); event.type = event.velocity === 0 ? "noteOff" : "noteOn"; } else if (command === 0x80) { event.note = this.readInt8(); event.velocity = this.readInt8(); event.type = "noteOff"; } else if (command === 0xc0 || command === 0xd0) this.readInt8(); else { this.readInt8(); this.readInt8(); } }
          if (event.type) events.push(event);
        }
        tracks.push(events); tracksFound++;
      } else this.pos += chunkLen;
    }
...
export class MidiPlayer {
  constructor(audioCtx) { this.audioCtx=audioCtx;this.gainNode=this.audioCtx.createGain();this.gainNode.connect(this.audioCtx.destination);this.gainNode.gain.value=.3;this.isPlaying=false;this.events=[];this.eventIndex=0;this.startTime=0;this.activeOscillators=[];this.schedulerTimer=null;this.lookahead=.1;this.scheduleAheadTime=.2;this.pausedTime=0; }
  get duration(){if(!this.events||this.events.length===0)return 0;return this.events[this.events.length-1].time;} get currentTime(){if(!this.isPlaying&&this.pausedTime>0)return this.pausedTime;if(!this.isPlaying)return 0;return Math.max(0,this.audioCtx.currentTime-this.startTime);}
  load(midiData){this.stop();this.events=this._mergeTracks(midiData);} _mergeTracks(midiData){const merged=[];midiData.tracks.forEach(track=>merged.push(...track));merged.sort((a,b)=>a.ticks-b.ticks);const result=[];let currentTicks=0,currentTime=0,microsecondsPerBeat=500000;const division=midiData.division;merged.forEach(event=>{const deltaTicks=event.ticks-currentTicks;const secondsPerTick=microsecondsPerBeat/1000000/division;currentTime+=deltaTicks*secondsPerTick;currentTicks=event.ticks;if(event.type==="tempo")microsecondsPerBeat=event.microsecondsPerBeat;else if(event.type==="noteOn"||event.type==="noteOff")result.push({time:currentTime,type:event.type,note:event.note,velocity:event.velocity,channel:event.channel});});return result;}

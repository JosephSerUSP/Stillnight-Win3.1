  ...
  parse() {
    ...
    while (tracksFound < nTracks && this.pos < this.data.length) {
      ...
      while (this.pos < startPos + chunkLen) {
        ticks += this.readVarInt(); let eventType = this.data[this.pos]; let event = { ticks };
...
export class MidiPlayer {
  ...
  _mergeTracks(midiData) {
    ...
    const result = [];
    let currentTicks = 0, currentTime = 0, microsecondsPerBeat = 500000;
    merged.forEach(event => {
      const deltaTicks = event.ticks - currentTicks;
      currentTime += deltaTicks * (microsecondsPerBeat / 1000000 / midiData.division);
  ...
  play(loop = true) {
    ...
    this.stop(); this.isPlaying = true; this.loop = loop; this.startTime = this.audioCtx.currentTime; this.eventIndex = 0; this.pausedTime = 0; this.schedule();
  }
  stop() {
    this.isPlaying = false; this.pausedTime = 0; clearTimeout(this.schedulerTimer);
    this.activeOscillators.forEach(osc => { try { osc.stop(); osc.disconnect(); } catch (_e) {} });
    this.activeOscillators = [];
  }
  pause() {
    if (!this.isPlaying) return;
    this.isPlaying = false; clearTimeout(this.schedulerTimer); this.pausedTime = this.audioCtx.currentTime - this.startTime;
    this.activeOscillators.forEach(osc => { try { osc.stop(); osc.disconnect(); } catch (_e) {} });
    this.activeOscillators = [];
  }
  ...
  schedule() {
    ...
    while (this.eventIndex < this.events.length && this.events[this.eventIndex].time + this.startTime < currentTime + this.scheduleAheadTime) {
      const event = this.events[this.eventIndex]; const playTime = this.startTime + event.time;
      if (playTime >= currentTime) { if (event.type === "noteOn") this.playNote(event.note, event.velocity, playTime); else if (event.type === "noteOff") this.stopNote(event.note, playTime); }
      this.eventIndex++;
    }
    if (this.eventIndex >= this.events.length) {
      if (this.loop) { const duration = this.events[this.events.length - 1].time; if (currentTime >= this.startTime + duration + 1.0) { this.startTime = currentTime; this.eventIndex = 0; } }
  ...
  playNote(note, velocity, time) {
    ...
    osc.type = "square"; osc.frequency.value = 440 * Math.pow(2, (note - 69) / 12); osc.connect(gain); gain.connect(this.gainNode); gain.gain.value = 0.5 * (velocity / 127); osc.start(time); this.activeOscillators.push(osc); osc._midiNote = note;
  }
  stopNote(note, time) {
    const index = this.activeOscillators.findIndex(o => o._midiNote === note);
    if (index !== -1) { const osc = this.activeOscillators[index]; osc.stop(time); this.activeOscillators.splice(index, 1); setTimeout(() => { try { osc.disconnect(); } catch (_e) {} }, (time - this.audioCtx.currentTime + 1) * 1000); }
  }
}

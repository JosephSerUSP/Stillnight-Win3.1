...
export class MidiPlayer {
  constructor(audioCtx) { this.audioCtx = audioCtx; this.gainNode = this.audioCtx.createGain(); this.gainNode.connect(this.audioCtx.destination); this.gainNode.gain.value = 0.3; this.isPlaying = false; this.events = []; this.eventIndex = 0; this.startTime = 0; this.activeOscillators = []; this.schedulerTimer = null; this.lookahead = 0.1; this.scheduleAheadTime = 0.2; this.pausedTime = 0; }
  get duration() { if (!this.events || this.events.length === 0) return 0; return this.events[this.events.length - 1].time; }

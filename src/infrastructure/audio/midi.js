  ...
  parse() {
    ...
    while (tracksFound < nTracks && this.pos < this.data.length) {
      ...
      while (this.pos < startPos + chunkLen) {
        ticks += this.readVarInt(); let eventType = this.data[this.pos]; const event = { ticks };
        if (eventType >= 0x80) { this.pos++; runningStatus = eventType; } else eventType = runningStatus;
        if (eventType === 0xff) {
          const metaType = this.readInt8(); const len = this.readVarInt();
          ...
          this.pos += len;
        } else if (eventType === 0xf0 || eventType === 0xf7) { this.pos += this.readVarInt(); }
        else {
...
/** WebAudio player for parsed MIDI data. */
export class MidiPlayer {
  constructor(audioCtx) { this.audioCtx = audioCtx; this.gainNode = audioCtx.createGain(); this.gainNode.connect(audioCtx.destination); this.gainNode.gain.value = 0.3; this.isPlaying = false; this.events = []; this.eventIndex = 0; this.startTime = 0; this.activeOscillators = []; this.schedulerTimer = null; this.lookahead = 0.1; this.scheduleAheadTime = 0.2; this.pausedTime = 0; }
  ...
  schedule() { if(!this.isPlaying)return; const now=this.audioCtx.currentTime; while(this.eventIndex<this.events.length && this.events[this.eventIndex].time+this.startTime<now+this.scheduleAheadTime){ const event=this.events[this.eventIndex]; const playTime=this.startTime+event.time; if(playTime>=now){ if(event.type==="noteOn")this.playNote(event.note,event.velocity,playTime); else this.stopNote(event.note,playTime); } this.eventIndex++; } if(this.eventIndex>=this.events.length){ if(this.loop){ const duration=this.events[this.events.length-1].time; if(now>=this.startTime+duration+1){this.startTime=now;this.eventIndex=0;} } else {this.stop();return;} } this.schedulerTimer=setTimeout(()=>this.schedule(),this.lookahead*1000); }
  playNote(note,velocity,time) { const osc=this.audioCtx.createOscillator(), gain=this.audioCtx.createGain(); osc.type="square"; osc.frequency.value=440*Math.pow(2,(note-69)/12); osc.connect(gain); gain.connect(this.gainNode); gain.gain.value=0.5*(velocity/127); osc.start(time); osc._midiNote=note; this.activeOscillators.push(osc); }
  stopNote(note,time) { const index=this.activeOscillators.findIndex(o=>o._midiNote===note); if(index===-1)return; const osc=this.activeOscillators[index]; osc.stop(time); this.activeOscillators.splice(index,1); setTimeout(()=>{try{osc.disconnect();}catch(_e){}},Math.max(0,time-this.audioCtx.currentTime+1)*1000); }
}

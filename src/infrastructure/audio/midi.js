...
export class MidiPlayer {
  ...
  schedule() {
    ...
    if (this.eventIndex >= this.events.length) {
      if (this.loop) {
        ...
        const loopTime = this.startTime + duration + 1.0;
        if (currentTime >= loopTime) { this.startTime = currentTime; this.eventIndex = 0; }
  ...
  playNote(note, velocity, time) {
    ...
    gain.gain.value = 0.5 * (velocity / 127);
    osc.start(time); this.activeOscillators.push(osc); osc._midiNote = note;
  }
  stopNote(note, time) {

...
export class SoundService {
  ...
  static async loadSound(key, path) {
    ...
    try {
      ...
      console.warn(`SoundService: Failed to load sound '${key}' from '${path}'.`, error);
    }
  }
  ...
  static async loadMidi(key, path) {
    ...
    try {
      ...
      console.warn(`SoundService: Failed to load midi '${key}' from '${path}'.`, error);
    }
  }
  ...
  static beep(frequency = 440, duration = 120) {
    ...
    try {
      ...
      oscillator.connect(gainNode); gainNode.connect(this._audioCtx.destination);
      oscillator.start(); oscillator.stop(this._audioCtx.currentTime + duration / 1000); oscillator.onended = onEnd;
    } catch (_e) { onEnd(); }
  }
}

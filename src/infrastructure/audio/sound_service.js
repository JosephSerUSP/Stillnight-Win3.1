...
export class SoundService {
  ...
  static async loadAll() {
    ...
    try {
      ...
      if (response.ok) {
        const text = await response.text(); const found = [];
        for (const match of text.matchAll(/href="([^"]+\.mid)"/g)) { const fileName = decodeURI(match[1]).split('/').pop(); if (fileName && !found.includes(fileName)) found.push(fileName); }
  ...
  static async loadSound(key, path) { if (!this._audioCtx) return; try { const response = await fetch(path); if (!response.ok) throw new Error(`HTTP error ${response.status}`); const audioBuffer = await this._audioCtx.decodeAudioData(await response.arrayBuffer()); this._buffers.set(key, audioBuffer); } catch (error) { console.warn(`SoundService: Failed to load sound '${key}' from '${path}'.`, error); } }

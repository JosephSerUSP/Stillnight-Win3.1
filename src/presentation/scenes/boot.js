...
export class Scene_Boot extends Scene_Base {
    ...
    async start() {
        ...
        if (this.contentLoader.sounds) await AudioAdapter.initialize(this.contentLoader.sounds);

        let session;
        const savedData = localStorage.getItem('stillnight_save_data');
        if (savedData) {
            try { session = SessionSerializer.fromJSON(JSON.parse(savedData)); }
            catch (e) { console.error("Failed to load save:", e); session = this._createNewSession(); }
        } else session = this._createNewSession();

        this.sceneManager.push(new Scene_Map(this.contentLoader, this.sceneManager, this.windowManager, session));
    }

    _createNewSession() {
        return { party: new Game_Party(), exploration: null, battle: null, interpreter: null, quests: new QuestLogState() };
    }
}

...
export class InterpreterSystem {
    ...
    _handleShop(state, command, _session) {
    ...
    _handleLog(_state, command, session) {
        const text = interpolateText(command.text, session);
        return [{ type: 'LOG', text: text }];
    }

    _handleBreakableWall(state, command, _session) {
        const event = state.activeEvent;
    ...
    _handleOfferQuest(state, command, _session) {
        return [{
            type: 'QUEST_OFFER',
    ...
    _handleCompleteQuest(state, command, _session) {
        return [{
            type: 'QUEST_COMPLETE',
            questId: command.questId,
            nextState: command.nextState,

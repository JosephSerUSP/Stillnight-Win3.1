...
export class InterpreterSystem {
    ...
    _handleBattle(state, _command, _session) {
    ...
    _handleShop(_state, command, _session) {
        return [{ type: 'SHOP_START', shopId: command.shopId }];
    }

    _handleRecruit(_state, command, _session) {
        return [{ type: 'RECRUIT_OPEN', options: command }];
    }

    _handleNpcDialogue(_state, command, _session) {
        return [{
    ...
    _handleDescend(_state, _command, _session) {
        return [{ type: 'DESCEND' }, { type: 'PLAY_SOUND', name: 'STAIRS' }];
    }

    _handleAscend(_state, _command, _session) {
        return [{ type: 'ASCEND' }, { type: 'PLAY_SOUND', name: 'STAIRS' }];
    }

    _handlePlaySound(_state, command, _session) {
    ...
    _handleGiveItem(_state, command, session) {
    ...
    _handleTakeItem(_state, _command, _session) {
         return null;
    }

    _handleGiveXp(_state, command, _session) {
        return [{
            type: 'GIVE_XP',
    ...
    _handleWait(state, command, _session) {
        state.waitMode = 'time';
    ...
    _handleWeightedBranch(state, command, _session) {
    ...
    _handleChoice(state, command, _session) {
        const events = [{
            type: 'SHOW_CHOICES',
    ...
    _handleSetVariable(_state, command, session) {
    ...
    _handleModifyVariable(_state, command, session) {
    ...
    _handleOfferQuest(_state, command, _session) {
        return [{
            type: 'QUEST_OFFER',
    ...
    _handleCompleteQuest(_state, command, _session) {
        return [{
            type: 'QUEST_COMPLETE',
            questId: command.questId,
            nextState: command.nextState,

...
export class BattleSystem {
  ...
  planRound(state, isFirstStrike = false) {
    ...
    const combatants = [];
    let partyMembers = [];
    if (state.participants.party.activeMembers) partyMembers = state.participants.party.activeMembers;
    else partyMembers = (state.participants.party.slots || []).slice(0, 5).filter(b => b);
    ...
    if (isFirstStrike) {
        ...
        const enemyQueue = sortBySpeed(plannedQueue.filter(c => c.isEnemy));
        const partyQueue = sortBySpeed(plannedQueue.filter(c => !c.isEnemy));
        state.turnQueue = [...enemyQueue, ...partyQueue];
    } else {

  ...
  openSettings() {
    ...
    const options = [
        ...
        },
        {
            label: "Audio",
            type: "toggle",
            value: ConfigManager.audioEnabled,
            onChange: (val) => {
                ConfigManager.audioEnabled = val;
                ConfigManager.save();
                SoundManager.checkConfig();
                if (val) SoundManager.play('UI_SELECT');
            }
        },
        {
            label: "Music",
            type: "toggle",
            value: ConfigManager.musicEnabled,
            onChange: (val) => {
                ConfigManager.musicEnabled = val;
                ConfigManager.save();
                SoundManager.checkConfig();
                // Resume map music if enabled, assuming we are on map
                if (val) {
                    this.checkMusic();
                    SoundManager.play('UI_SELECT');
                }
        ...
        },
        {
            label: "SFX",
            type: "toggle",
            value: ConfigManager.sfxEnabled,
            onChange: (val) => {
                ConfigManager.sfxEnabled = val;
                ConfigManager.save();
                SoundManager.checkConfig();
                if (val) SoundManager.play('UI_SELECT');
            }

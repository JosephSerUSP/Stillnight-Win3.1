    ...
    startRecruit(event, action) {
        ...
        if (event._recruitData) {
            ...
            if (!recruitId) {
                ...
                const floor = this.map.floors[this.map.floorIndex];
                if (floor.recruits && floor.recruits.length > 0) {
                    recruitId = floor.recruits[randInt(0, floor.recruits.length - 1)];
                } else {
                    // Global fallback
        ...
        const actorData = this.dataManager.actors.find(a => a.id === recruitId);

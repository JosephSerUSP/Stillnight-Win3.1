...
The roadmap is complete. `TraitManager` has been migrated to `TraitRules` and `EncounterManager` to `EncounterSystem`. The "Remove remaining legacy knot" phase is finished.
...
*   Migrate `EncounterManager` to `EncounterSystem` (Complete — EncounterManager removed; EncounterSystem created in src/engine/systems/).

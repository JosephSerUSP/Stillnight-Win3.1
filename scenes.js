...
export class Scene_Map extends Scene_Base {
  ...
  updateAll() {
    ...
    this.updateCardList();
    this.updateParty();
    this.hud.updateStatus(this.party.gold, this.party.inventory.length, this.runActive);
  ...
  updateGrid() {
    ...
    this.hud.drawGrid(this.map.MAX_W, this.map.MAX_H, (x, y) => {
        ...
        if (!visited && !isPlayer) {
          ...
          if (event) {
              ...
              if (visible) {
                  symbol = event.symbol;
                  if (event.cssClass) cssClass.push(event.cssClass);
              }
          }

...
export class Scene_Battle extends Scene_Base {
  ...
  async animateEvents(events) {
      ...
      for (const event of events) {
            ...
            } else if (event.type === 'death' && event.target) {
    ...
    return new Promise((resolve) => {
      ...
      const interpolator = () => {
        ...
        if (progress < 1) {
          ...
          // Final sync - skipped if dead to prevent premature removal
          if (battler.hp > 0) {
              this.renderBattleAscii();
          }
          resolve();
  ...
  /**
   * @method animateDeath
   * @description Animates the death of a battler.
   * @param {Game_Battler} battler - The battler to animate death for.
   */
  animateDeath(battler) {
      return new Promise((resolve) => {
          // Find element
          const enemyIndex = this.battleManager.enemies.indexOf(battler);
          const partyIndex = this.party.members.indexOf(battler);
          ...
          const collapseGauge = () => {
              return new Promise((res) => {
                  if (!hpEl) { res(); return; }
                  // Animate gauge collapsing: "[          ]" -> "[]"
                  // Actually, if we use standard text, we can just reduce spaces.
                  // Current content is likely "[          ]" (all spaces since HP is 0)
                  // Or whatever createHpGauge returned.
                  // We want to animate reducing the inner width.

                  let width = 15; // default length in createHpGauge
                  // Actually we should read it, but let's just animate a string reducing in length.
                  const interval = 50;

                  const collapse = () => {
                      width--;
                      if (width < 0) width = 0;
                      hpEl.textContent = `[${" ".repeat(width)}]`;
          ...
          collapseGauge().then(() => {
              // Flicker whole container
              container.classList.add('blink');
              setTimeout(() => {
                  container.classList.remove('blink');
                  // Disappear
                  container.style.display = 'none'; // Or opacity 0
                  resolve();
              }, 300); // Wait for blink to finish (blink anim is 0.1s x 2)
          });
      });
  }

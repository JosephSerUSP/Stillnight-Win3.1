...
export class Scene_Map extends Scene_Base {
    ...
    gameContainer.innerHTML = `
      <div class="stack-nav panel">
        ...
        <div class="group-box">
          ...
          <div style="margin-top:4px; border-top:1px solid #777; padding-top:4px;">
            <div style="font-weight:bold;">Objective:</div>
            <div id="objective-label" style="font-style:italic; font-size:11px;">Explore...</div>
          </div>
        </div>
        <div class="group-box">
  ...
  getDomElements() {
    ...
    this.objectiveLabelEl = document.getElementById("objective-label");
  ...
  updateObjective() {
      const floor = this.map.floors[this.map.floorIndex];
      if (floor && floor.objective) {
          this.objectiveLabelEl.textContent = floor.objective;
      } else {
          this.objectiveLabelEl.textContent = "Explore the depths.";
      }
  }
  ...
  onTileClick(e) {
    ...
    if (event) {
        ...
        if (!isHidden && (event.blocking || event.type === 'enemy' || event.type === 'boss' || (event.type === 'stairs' && event.locked))) {
            isBlocking = true;
        }
    }
    ...
    if (event) {
       if (isHidden) {
           // Reveal hidden event (trap/story)
           event.hidden = false;
           this.currentInteractionEvent = event;
           this.executeEvent(event);
           return;
       } else {
           // Walked on non-blocking event (unlocked stairs, item, etc)
           this.currentInteractionEvent = event;
           this.executeEvent(event);
           return;
       }
  ...
  executeAction(action, event) {
      switch(action.type) {
          ...
          case 'SEARCH':
              ...
              break;
          case 'ENDING':
              this.triggerEnding(action);
              break;
      }
  }

  triggerEnding(action) {
      this.eventWindow.show({
          title: action.title || "The End",
          description: action.text || "You have broken the cycle.",
          image: action.image || "ending.png",
          style: 'terminal',
          choices: [{
              label: "Finish",
              onClick: () => {
                  ...
                  this.logMessage("Thank you for playing.");
                  this.windowManager.push(this.eventWindow);
                  this.eventWindow.show({
                      title: "Credits",
                      description: "Game Design & Code: Jules\n\nPlay again?",
                      choices: [{ label: "New Run", onClick: () => { this.closeEvent(); this.startNewRun(); } }]
                  });
              }
          }]
      });

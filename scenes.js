          ...
          if (effect.type === "hp_damage") {
            ...
            events.push({
              ...
              apply: () => {
                ...
                const lifesteal = p.getPassiveValue("LIFESTEAL_AURA");
                if (lifesteal > 0) {
                  const targetIndex = index % 2 === 0 ? index + 1 : index - 1;
                  if (targetIndex >= 0 && targetIndex < 4) {
                    const target = this.party.members[targetIndex];
                    if (target && target.hp > 0) {
                      target.hp = Math.min(target.maxHp, target.hp + lifesteal);
                      this.logMessage(`[Passive] ${p.name} heals ${target.name} for ${lifesteal} HP.`);
                    }
                  }
                }
                return { battler: target, oldHp };

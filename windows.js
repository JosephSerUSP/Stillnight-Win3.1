...
export function createPartySlot(member, index) {
    ...
    const xpPercent = xpNeeded > 0 ? (member.xp / xpNeeded) * 100 : 0;
    xpFill.style.width = `${Math.min(100, Math.max(0, xpPercent))}%`;
    xpBar.appendChild(xpFill);

    // Equipment
    const equipLabel = document.createElement("div");
    equipLabel.className = "party-slot-equip";

    let itemData = null;
    if (member.equipmentItem) {
        itemData = member.equipmentItem;
    } else if (member.baseEquipment) {
        itemData = typeof member.baseEquipment === 'string'
            ? { name: member.baseEquipment }
            : member.baseEquipment;
    }

    if (itemData) {
        const label = createInteractiveLabel(itemData, 'item');
        equipLabel.appendChild(label);
    } else {
        const noneSpan = document.createElement("span");
        noneSpan.textContent = "None";
        noneSpan.style.color = "#808080";
        equipLabel.appendChild(noneSpan);
    }
 ...
 * @param {string} type - The type of object ('item', 'skill', 'passive').
...
/**
 * @class WindowLayer
 * @description Manages the positioning and z-indexing of windows.
 */
export class WindowLayer {
   ...
   * @param {Window_Base} window - The window to add.
   */
  addChild(window) {
  ...
  /**
   * Appends the layer to a parent DOM element.
...
/**
 * @class WindowManager
 * @description Manages the stack of active windows and handles input delegation.
 */
export class WindowManager {
   ...
   * @param {Window_Base} window - The window to push.
   */
  push(window) {
   ...
   * @returns {Window_Base|null} The popped window, or null if empty.
   */
  pop() {
  ...
  /**
   * Closes a specific window instance.
  ...
  /**
   * Handles global input events, delegating to the top window.
  ...
  /**
   * Updates visual state (dimming) based on stack position.
   */
  updateState() {
...
export class Window_Base {
     ...
     * @param {number|string} x - X position or 'center'.
     * @param {number|string} y - Y position or 'center'.
 ...
 * @extends Window_Base
 * @description A base window class for displaying lists of items with tabs.
 */
 ...
 * @description Displays the party's inventory with tabs for consumables and equipment.
 */
 ...
 * @extends Window_ItemList
 * @description A popup window for selecting an equipment item for a specific slot.
 */
 ...
 * @extends Window_Base
 * @description A popup window for selecting a party member target.
 */
